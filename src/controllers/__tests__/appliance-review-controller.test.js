...
describe('appliance-review-controller', () => {
  ...
  describe('updateApplianceReview', () => {
    ...
    test('marks a new review as in progress', async () => {
      collection.findOne
        .mockResolvedValueOnce({ technicalReview: { status: 'new' } })
        .mockResolvedValueOnce({ id: 'APP-1' })
      collection.updateOne.mockResolvedValue({ matchedCount: 1 })
    ...
    test('saving progress does not gate on outstanding checks', async () => {
      collection.findOne
        .mockResolvedValueOnce({ technicalReview: { status: 'new' } })
        .mockResolvedValueOnce({ id: 'APP-1' })
      collection.updateOne.mockResolvedValue({ matchedCount: 1 })
    ...
    test('saving progress does not undo an accepted decision', async () => {
      collection.findOne.mockResolvedValue({
        technicalReview: {
          status: 'accepted',
    ...
    test('saving progress does not undo a rejected decision', async () => {
      collection.findOne.mockResolvedValue({
        technicalReview: { status: 'rejected', reviewedAt: null }
      })
